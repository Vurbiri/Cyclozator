var YandexPlugin =
{
    ...
    IsPaymentsJS: function () { return func.isPayments(); },
    ...
    GetPlayerIdJS: function () {
        var id;
        if (func.isLogOn())
            id = vars.player.getUniqueID();
        else
            id = '';

        return func.toUnityString(id);
    },
    ...
    InitPaymentsJS: function () {

        if (!func.isInitialize()) {
            console.log("-- InitPayments: not logon --");
            window.unityInstance.SendMessage('YMoney', 'OnEndInitPayments', 0);
            return;
        }
        ...
        vars.ysdk
            ...
            .then(_payments => {
                vars.payments = _payments;
                console.log("++ InitPayments ++");
                window.unityInstance.SendMessage('YMoney', 'OnEndInitPayments', 1);
            })
            .catch(err => {
                vars.payments = null;
                console.log("-- InitPayments: " + err + "  --");
                window.unityInstance.SendMessage('YMoney', 'OnEndInitPayments', 0);
            });
    },
    PurchaseJS: function (pId, devPayload) {

        if (!func.isPayments()) {
            console.log("-- Purchase: not payments --");
            window.unityInstance.SendMessage('YMoney', 'OnEndPurchase', '');
            return;
        }

        var sId = UTF8ToString(pId);
        var sDevPayload = UTF8ToString(devPayload);
        vars.payments
            .purchase({ id: sId, developerPayload: sDevPayload })
            .then(_pur => {
                 if (func.isEmpty(_pur)) {
                     console.log("++ Purchase: NULL ++");
                     window.unityInstance.SendMessage('YMoney', 'OnEndPurchase', '');
                 } else {
                     var purchase = { productID: _pur.productID, purchaseToken: _pur.purchaseToken, developerPayload: _pur.developerPayload };
                     var json = JSON.stringify(purchase);
                     console.log("++ Purchase ++");
                     window.unityInstance.SendMessage('YMoney', 'OnEndPurchase', json);
                 }
            })
            .catch(err => {
    ...
    GetPurchasesJS: function () {

        if (!func.isPayments()) {
            console.log("-- GetPurchases: not payments --");
            window.unityInstance.SendMessage('YMoney', 'OnEndGetPurchases', '');
            return;
        }
        ...
        vars.payments
            ...
            .then(_pur => {
                if (func.isEmpty(_pur)) {
                    console.log("++ GetPurchases: NULL ++");
                    window.unityInstance.SendMessage('YMoney', 'OnEndGetPurchases', 'Empty');
                } else {
                    var purchase = Array.from(_pur, (p) => { return { productID: p.productID, purchaseToken: p.purchaseToken, developerPayload: p.developerPayload } });
                    var json = JSON.stringify(purchase);
                    console.log("++ GetPurchases ++");
                    window.unityInstance.SendMessage('YMoney', 'OnEndGetPurchases', json);
                }
            })
            .catch(err => {
    ...
    ConsumePurchaseJS: function (token) {

        if (!func.isPayments()) {
            console.log("-- ConsumePurchase: not payments --");
            window.unityInstance.SendMessage('YMoney', 'OnEndConsumePurchase', 0);
            return;
        }

        var sToken = UTF8ToString(token);
        vars.payments
            .consumePurchase(sToken)
            .then(() => {
                console.log("++ ConsumePurchase ++");
                window.unityInstance.SendMessage('YMoney', 'OnEndConsumePurchase', 1);
            })
            .catch(err => {
                console.log("-- ConsumePurchase: " + err + "  --");
                window.unityInstance.SendMessage('YMoney', 'OnEndConsumePurchase', 0);
            });
    },
    ...
    $func: {
        ...
        isLeaderboard: function () { return func.isPlayer() && !func.isEmpty(vars.lb); },
        isPayments: function () { return func.isInitialize() && !func.isEmpty(vars.payments); },
